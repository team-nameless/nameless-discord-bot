...
class ModeratorSlashCog(commands.Cog):
    def __init__(self, bot: commands.AutoShardedBot) -> None:
        self.bot = bot

    @nextcord.slash_command(description="Moderation commands", guild_ids=Config.GUILD_IDs)
    async def mod(self, _: nextcord.Interaction):
        pass

    @staticmethod
    async def __generic_ban_kick(interaction: nextcord.Interaction, reason: str, action: str, caller, d=-1):
        client: nextcord.Client = interaction.client

        await interaction.response.defer()
        await interaction.edit_original_message(content=f"Mention members you want to {action} with reason {reason}")
        msg: nextcord.Message = await client.wait_for("message",
                                                      check=Utility.message_waiter(interaction), timeout=30)
    ...
    @staticmethod
    async def __generic_warn(interaction: nextcord.Interaction, member: nextcord.Member, reason: str, val: int,
    ...
    @staticmethod
    async def __generic_mute(interaction: nextcord.Interaction, member: nextcord.Member, reason: str, mute: bool = 1):
        await interaction.response.defer()
        mute_role, is_new = await Utility.get_or_create_role(interaction, MUTE_ROLE_NAME, "Mute role creation")
    ...
    async def ban(self, interaction: nextcord.Interaction,
        ...
        if not 0 <= delete_message_days <= 7:
            ...
            await self.__generic_ban_kick(interaction, reason, "ban", interaction.guild.ban, delete_message_days)
    ...
    async def warn_add(self, interaction: nextcord.Interaction,
        ...
        async def diff_fn(i: nextcord.Interaction, m: nextcord.Member, r: str, current: int, prev: int):
            pass

        async def three_fn(i: nextcord.Interaction, m: nextcord.Member, r: str):
            role, no = await Utility.get_or_create_role(interaction=i, name=MUTE_ROLE_NAME, reason="Mute role creation")
        ...
        await ModeratorSlashCog.__generic_warn(interaction, member, reason, 1, zero_fn, three_fn, diff_fn)
    ...
    async def warn_remove(self, interaction: nextcord.Interaction,
        ...
        async def diff_fn(i: nextcord.Interaction, m: nextcord.Member, r: str, current: int, prev: int):
            if prev == 3:
                role, _ = await Utility.get_or_create_role(interaction=i, name=MUTE_ROLE_NAME,
    ...
    async def mute(self, interaction: nextcord.Interaction,
                   member: nextcord.Member = SlashOption(description="Target member"),
                   reason: str = SlashOption(description="Reason to mute", default="Rule violation")):
    ...
    async def unmute(self, interaction: nextcord.Interaction,
                     member: nextcord.Member = SlashOption(description="Target member"),
                     reason: str = SlashOption(description="Reason to unmute", default="Good behavior")):
