...
from nextcord.ext.menus import Menu, PageFormatType
from ossapi import *
import nextcord
...
class OsuScoresPageSource(menus.ListPageSource):
    def __init__(self, data):
        super().__init__(data, per_page=1)

    async def format_page(self, menu: Menu, entries: List[Score]) -> PageFormatType:
...
class OsuSlashCog(commands.Cog):
    ...
    async def __generic_check(self, interaction: nextcord.Interaction, request: str, username: str, mode: str,
        ...
        if request == "profile":
            ...
            await interaction.edit_original_message(embeds=[eb])
        else:
            """
            ...
            try:
                msg: nextcord.Message = self.bot.wait_for("message",
                ...
                try:
                    ...
                    if not is_from_context:
                        ...
                        await interaction.edit_original_message(content="Do you want to include fail scores?",
                                                                view=view)
                        await view.wait()
                    ...
                    scores = self.api.user_scores(osu_user.id, request_type, include_fails, the_mode, limit)

                except ValueError:
                    await interaction.followup.send(content="Invalid number provided. Please run the command again and"
                                                            "correct the input.")
            except TimeoutError:
                await interaction.followup.send(content="Timed out")
            """
            await interaction.edit_original_message(content="Not implemented")
            return
