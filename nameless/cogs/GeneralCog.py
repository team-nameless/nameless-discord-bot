...
class GeneralCog(commands.Cog):
    ...
    async def the_bot(self, interaction: discord.Interaction):
        ...
        embed: discord.Embed = (
            ...
            .add_field(name="⭐ Biography", value=NamelessConfig.__description__.replace('{github_link}', github_link), inline=False)
            .add_field(name="🫡 Service status", value=f"Serving {servers_count} servers for a total of {total_members_count} users.", inline=False)
            ...
            .add_field(name="💻 Runtime", value=f"**discord.py {discord.__version__}** on **{python_implementation()} {python_version()}**")
        )

        buttons = discord.ui.View()

        if interaction.client.application.bot_public:
            buttons.add_item(discord.ui.Button(label="Invite me!", style=discord.ButtonStyle.url, url=bot_inv, emoji="😳"))

        if support_inv:
            buttons.add_item(discord.ui.Button(label="Support server", style=discord.ButtonStyle.url, url=support_inv, emoji="🤝"))

        buttons.add_item(discord.ui.Button(label="Source code", style=discord.ButtonStyle.url, url=github_link, emoji="📃"))
