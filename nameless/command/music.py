...
class MusicCommands(commands.GroupCog, name="music"):
    ...
    async def connect_nodes(self):
        ...
        logging.info("Wait for discord connection to be ready before connect to Lavalink...")
    ...
    async def on_pomice_track_start(self, player: CustomPlayer, track: pomice.Track):
        if not player.guild:
            ...
            return
        if not player.play_now_allowed and player.queue.loop_mode is not pomice.LoopMode.QUEUE:
            return

        embed = self.generate_embed_from_track(player, track, self.bot.user)
        await player.send_to_channel(embed=embed)

    @commands.Cog.listener()
    async def on_wavelink_inactive_player(self, player: CustomPlayer):
        await player.send_to_channel("I have been inactive for a while. Goodbye!")
        await player.disconnect()

    @commands.Cog.listener()
    async def on_voice_state_update(self, member: discord.Member, _: discord.VoiceState, after: discord.VoiceState):
        if self.bot.user is not None and member.id == self.bot.user.id and not after.deaf:
    ...
    def generate_embeds_from_tracks(
        self,
        tracks: pomice.Queue | list[pomice.Track] | pomice.Playlist | None,
        embed_title: str = "Tracks currently in queue",
    ) -> list[discord.Embed]:
        """Generate embeds from supported track list types."""
        txt = ""
        embeds: list[discord.Embed] = []
        track_list: list[pomice.Track] = []

        if isinstance(tracks, pomice.Queue):
            track_list = tracks._queue.copy()
        ...
        for i, track in enumerate(track_list, start=1):
            upcoming = f"{i} - [{track.title} by {self.resolve_artist_name(track.author)}]({track.uri or 'N/A'})\n"

            if len(txt) + len(upcoming) > 2048:
                embeds.append(discord.Embed(title=embed_title, color=discord.Color.orange(), description=txt))
                txt = upcoming
            else:
                txt += upcoming
    ...
    def generate_embed_from_track(
        ...
        embed = (
            ...
            .add_field(
                name="Source",
                value=f"[{str.title(track.track_type.value)}]({track.uri})" if track.uri else "N/A",
                inline=False,
            )
            # .add_field(
            #     name="Looping",
        ...
        if player.queue.loop_mode != pomice.LoopMode.TRACK and not track.is_stream and bool(player.queue):
            next_tr = player.queue[0]
            embed.add_field(
                name="Next track",
                value=(
                    f"[{escape_markdown(next_tr.title) if next_tr.title else 'Unknown title'} "
    ...
    @staticmethod
    async def show_paginated_tracks(ctx: commands.Context[Nameless], embeds: list[discord.Embed]):
    ...
    @overload
    async def ensure_guild_voice_client(self, ctx: commands.Context[Nameless]) -> CustomPlayer: ...
    @overload
    ...
    async def _connect(
        ...
        try:
            if channel is not None:
                await channel.connect(self_deaf=True, cls=CustomPlayer)  # type: ignore
    ...
    async def connect(
        ...
        await ctx.defer()
        success = await self._connect(ctx, channel)
        if not success:
            raise commands.CommandError("Failed to connect to the voice channel.")
    ...
    async def pick_track_from_results(
        self, ctx: commands.Context[Nameless], tracks: list[pomice.Track]
    ) -> list[pomice.Track]:
        if len(tracks) == 1:
            return tracks

        view = discord.ui.View().add_item(TrackDropdown([track for track in tracks if not track.is_stream]))
        ...
        pick_list: list[pomice.Track] = [tracks[int(val)] for val in vals]
        return pick_list
    ...
    @app_commands.choices(origin=[app_commands.Choice(name=k, value=k) for k in SOURCE_MAPPING])
    async def play(self, ctx: commands.Context[Nameless], query: str, flags: PlayFlags) -> None:
        """Play a song."""
        await ctx.defer()
        ...
        tracks: list[pomice.Track] | pomice.Playlist | None = await player.get_tracks(
            query, search_type=SOURCE_MAPPING[flags.origin]
        )
        ...
        if position_to_add == 0:
            player.queue.extend(soon_added)
        else:
            player.queue._queue[position_to_add:position_to_add] = soon_added
    ...
    async def pause(self, ctx: commands.Context[Nameless]) -> None:
        ...
        await ctx.send(embed=self.generate_embed_from_track(player, player.current, ctx.author))
    ...
    async def queue(self, ctx: commands.Context[Nameless]) -> None:
        ...
        embeds = [
            ...
            *self.generate_embeds_from_tracks(player.auto_queue, "Autoplay track queue"),
        ]
    ...
    async def current(self, ctx: commands.Context[Nameless]) -> None:
        ...
        player = await self.ensure_guild_voice_client(ctx, auto_connect=True)
        track: pomice.Track | None = player.current  # type: ignore
    ...
    class SeekFlags(commands.FlagConverter):
        milliseconds: int = commands.flag(default=0, description="Milisecond component of position", aliases=["ms"])
        ...
        minutes: int = commands.flag(default=0, description="Minute component of position", aliases=["m"])
        hours: int = commands.flag(default=0, description="Hour component of position", aliases=["h"])
    ...
    async def autoplay(self, ctx: commands.Context[Nameless], mode: str | None = None):
        """Change autoplay mode."""
        player = await self.ensure_guild_voice_client(ctx, auto_connect=True)
        mode_bool = mode == "enable"

        if mode is None:
            await ctx.send(f"Autoplay mode is currently set to {mode}.")
            return

        if player.is_autoplaying is mode_bool:
        ...
        if mode_bool:
            ...
            if not player.auto_queue:
                await ctx.send("Failed to fetch autoplay tracks. Autoplay will be disabled.")
                return
        ...
        player.is_autoplaying = mode_bool
        await ctx.send(f"Autoplay mode has been set to {mode}.")
